...
to move-turtles
  ...
  ask turtles [
    ...
    ;Update all others directions with

    foreach (n-values number-of-angles [ i -> i ])
      [ x ->
        if(x != head-i)
        [
          let cPayoff (array:item directions-reward x)
          array:set directions-reward x  linear-updating-model cPayoff discounting-factor 0
        ]
    ]
  ]
end
;todo add tremble
to-report linear-updating-model [cumulative-payoff discount-factor current-payoff ]
  report (1 - discount-factor) * cumulative-payoff + current-payoff
end
